...
class UserService {
  ...
  Future signin(User user) {
    return signout()
      .then((_) => _http.post('${window.location.origin}/login', 'username=${user.username}&password=${user.password}', headers: { 'Content-Type' : 'application/x-www-form-urlencoded'})
      .then((HttpResponse response) => _client.connectWebsocket()
      .then((_) => getAuthenticatedUser().then((User u) {
    ...
    Future<User> signup(User user) {
      return signout()
          .then((_) => _client.connectWebsocket())
          .then((_) => _client.jsonMessageRequest("/app/usr/signup", user, "/user/queue/user-created", (_) => new User.fromJsonMap(_)))
